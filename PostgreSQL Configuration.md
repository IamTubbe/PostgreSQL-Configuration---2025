...
2. อธิบายผลการทดลอง การ SELECT , INSERT, UPDATE, DELETE เป็นอย่างไร 
    - ผลการทดลองแสดงว่า SELECT (JOIN + WHERE) และ INSERT ทำงานเร็วมาก (เฉลี่ยไม่ถึง 0.05 ms ต่อรอบ) ส่วน UPDATE และ DELETE (soft) ใช้เวลานานกว่า (~0.58–0.63 ms ต่อรอบ) เนื่องจากต้องแก้ไขข้อมูลเดิมและตรวจสอบ row, แสดงว่าการอ่านง่ายกว่าเขียน/อัปเดต
```
```sql
